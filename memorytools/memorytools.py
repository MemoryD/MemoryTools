...
class MemoryTool(object):
    ...
    def about(self, sysTrayIcon: SysTrayIcon):
    ...
    def createMenu(self):
        '''创建托盘程序的菜单'''
        self.menu_options = (
                            ('复制翻译', getSrc('trans.ico'), self.ct.createMenu(), True),
                            ('OCR识别', getSrc('ocr.ico'), self.ocr.createMenu(), True),
                            ('休息提醒', getSrc('alert.ico'), self.alert.createMenu(), True),
                            ('关于', getSrc('about.ico'), self.about, True),
                            )
